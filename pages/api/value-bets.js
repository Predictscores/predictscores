...
// SAMO named kvote (bookmaker != "aggregate") — agregatni izvori se ignorišu.
// Filter: ban liga/zemlja/ime tima, one-per-fixture, cap po ligi, limit.
...
// BAN regex (liga, zemlja, TIM)
const BAN_LEAGUES_RE = safeRegex(
  process.env.BAN_LEAGUES ||
...
export default async function handler(req, res) {
  try {
    ...
    for (const fx of slotFixtures) {
      ...
      // HARD: nikad ne koristimo aggregate
      let cand = candAll.filter((p) => p.bookmaker && p.bookmaker !== "aggregate");

      // trusted-only preferencija + filter
      if (trustedOnly) {
        const trustedList = getTrustedList();
        if (trustedList.length) {
          const namedTrusted = cand.filter((p) => trustedList.includes(String(p.bookmaker).toLowerCase()));
          cand = namedTrusted.length ? namedTrusted : [];
        }
      }
    ...
    if (!perFixtureBest.length) {
      return res.status(200).json({ ok: true, disabled: false, slot, value_bets: [], source: "no-named>=min" });
    }
    ...
    return res.status(200).json({ ok: true, disabled: false, slot, value_bets: capped.slice(0, qpLimit), source: "named-only(filtered)" });
  } catch (e) {
...
function candidateOddsKeys(ymd, fid) {
  return [`odds:${ymd}:${fid}`, `odds:fixture:${ymd}:${fid}`, `odds:${fid}`, `odds:fixture:${fid}`];
}
...
function isTrusted(name) {
  ...
  return getTrustedList().includes(String(name).toLowerCase());
}
...
function coalesce(picks) {
  ...
  for (const p of picks) {
    ...
    const key = `${String(p.market).toUpperCase()}|${String(p.selection).toUpperCase()}|${String(p.bookmaker).toLowerCase()}`;
...
function pickBestForFixture(arr) {
  ...
  for (const p of arr) {
    ...
    const score = (p.trusted ? 2000 : 0) + conf + (Number(p.price) * 0.01);
...
function confidenceFromPrice(price) {
  ...
  const cap = 85; // bez agregata nam ne treba 68-cap
...
function toTicket(p, fx) {
  ...
  return {
    ...
    confidence_pct: conf,
    explain: { bullets: [p.bookmaker ? `source: ${p.bookmaker}` : "source: named"] },
