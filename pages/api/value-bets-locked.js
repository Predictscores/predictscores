...
function ymdInTZ(d=new Date(), tz=TZ){
  const f=new Intl.DateTimeFormat("en-CA",{timeZone:tz,year:"numeric",month:"2-digit",day:"2-digit"});return f.format(d);
}
...
function normalize(it){
  ...
  if(typeof raw==="string"){ pickStr=["1","X","2"].includes(raw)?labelFor(raw):raw; }
  else if(raw&&typeof raw==="object"){ code=code||raw.code; pickStr=raw.label||it?.selection_label||labelFor(code); }
  else { pickStr=it?.selection_label||labelFor(code); }
  const home=it.home||it?.teams?.home||""; const away=it.away||it?.teams?.away||"";
  const league_name=it.league_name||it?.league?.name||""; const league_country=it.league_country||it?.league?.country||"";
  const price=it?.odds?.price ?? it?.price ?? null; const books_count=it?.odds?.books_count ?? it?.books_count ?? null;
...
export default async function handler(req,res){
  try{
    ...
    const arr = full ? await kvGet(keyFull) : await kvGet(keySlim);
    const base = Array.isArray(arr) ? arr : (Array.isArray(await kvGet(keyFull)) ? await kvGet(keyFull) : []);
