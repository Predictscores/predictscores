...
export default async function handler(req,res){
  try{
    ...
    // prefer vbl_full (koji sada NIJE cap-ovan)
    const tried=[];
    ...
    // sortiraj, pa selektuj do cap-a uz max 2/league (UEFA ≤6)
    function isUefa(lg){ const n=String(lg?.name||""); const c=String(lg?.country||""); return /UEFA|Champions League|Europa League|Conference League|European Championship|Euro Qual/i.test(n)||/Europe/i.test(c); }
    const perLeague=new Map(); const picked=[];
    for(const it of filtered.slice().sort(byConfKick)){
      const lg=it?.league||{}; const key=String(lg.id||lg.name||"?");
      const limit=isUefa(lg)?6:2;
      const cur=perLeague.get(key)||0; if(cur>=limit) continue;
      perLeague.set(key,cur+1);
      const kd=it?.fixture?.date||it?.kickoff_utc||it?.kickoff||null;
      picked.push({...it, kickoff: kd || null});
      if(picked.length>=cap) break;
    }

    // tiketi – kao i ranije (živi u slot ključu)
    ...
    return res.status(200).json({ok:true,slot,ymd,items:picked,tickets,source:srcKey,policy_cap:cap,slot_cap:cap,min_odds:MIN_ODDS,debug:{reads:tried}});
