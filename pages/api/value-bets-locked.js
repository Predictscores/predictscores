...
const weekdayShort = (d, tz) => new Intl.DateTimeFormat("en-US", { timeZone: tz, weekday:"short" }).format(d);
const isWeekendYmd = (ymd, tz) => {
  const [y,m,dd] = ymd.split("-").map(Number);
  const dt = new Date(Date.UTC(y, m-1, dd));
  const wd = weekdayShort(dt, tz);
...
const kickoffTs = it => {
  const k = it?.fixture?.date || it?.fixture_date || it?.kickoff || it?.kickoff_utc || it?.ts;
...
export default async function handler(req, res) {
  try {
    ...
    // 1) učitaj slot feed (prefer vbl_full)
    const tried = [];
    async function firstHit(keys) {
      for (const k of keys) {
        const { raw } = await kvGETraw(k);
        tried.push({ key:k, hit: !!raw });
        const arr = J(raw) || (J(J(raw)?.value||"")||[]);
    ...
    // Ako ničega nema, vrati prazan ali jasan odgovor (frontend će prikazati "no data")
    if (!base.length) {
      const { raw: tRaw } = await kvGETraw(`tickets:${ymd}:${slot}`);
      const t = J(tRaw) || {};
      return res.status(200).json({ ok:true, slot, ymd, items:[], tickets: t, source: srcKey, note:"empty-slot-feed" });
    }

    // 2) sort + cap, NE filteriši po odds.price (može biti null) – UI ga ne zahteva
    const items = base.slice().sort(byConfKick).slice(0, cap);

    // 3) tiketi: per-slot → dnevni → prazno (ne čistimo ih)
    const { raw:rawS } = await kvGETraw(`tickets:${ymd}:${slot}`);
    let tickets = J(rawS) || null;
    if (!tickets) {
      const { raw:rawD } = await kvGETraw(`tickets:${ymd}`);
      tickets = J(rawD) || { btts:[], ou25:[], htft:[] };
    }
