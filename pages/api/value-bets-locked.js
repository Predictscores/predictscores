...
function countTickets(t){ return (t?.btts?.length||0)+(t?.ou25?.length||0)+(t?.htft?.length||0); }

/* ---------------- tickets reader (zamrznuti + fallback ako prazno) ---------------- */
async function readTicketsOrDerive(ymd, slot, trace, itemsForFallback){
  const tried = [];
  ...
  for (const k of keys){
    ...
    if (obj && typeof obj === "object") {
      ...
      const t = { btts, ou25, htft };
      // ⬇️ KLJUČNO: ako su prazni → deriviraj umesto da vratiš prazno
      if (countTickets(t) > 0) return { tickets: splitMarkets([...btts, ...ou25, ...htft]), source: k, tried };
      break; // postoji ključ ali je prazan → izaći i derivirati
    }
  }

  // derivacija: probaj union ako ga imamo; ako ne, koristi items (već učitane)
  let baseArr = Array.isArray(itemsForFallback) ? itemsForFallback : [];
  if (!baseArr.length) {
    const { raw } = await kvGETraw(`vb:day:${ymd}:union`, trace);
    const arr = arrFromAny(J(raw));
    if (Array.isArray(arr) && arr.length) baseArr = arr;
  }
  return { tickets: splitMarkets(baseArr), source: "derived-from-items", tried };
}
...
export default async function handler(req,res){
  try{
    ...
    // 2) tickets (KV → ako prazno, deriviraj)
    const t0 = await readTicketsOrDerive(ymd, slot, trace, items);
    const tickets = t0.tickets;
    const tsrc = t0.source;
