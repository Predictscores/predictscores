...
async function kvSetJSON(key, value){
  if (DRY_RUN) return true; // opcioni "suvi hod"
  const base  = process.env.KV_REST_API_URL || process.env.KV_URL || process.env.UPSTASH_REDIS_REST_URL;
  const token = process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN;
...
// 1X2 extraction (radi i sa oblikom gdje postoji "bookmakers[]")
...
export default async function handler(req, res){
  ...
  const candidates = payload.arr.filter(r => {
    ...
    // fallback: probaj "kickoff" (tretiraj kao lokalno, ali poredi približno u UTC parsiranjem)
  ...
  const updatesById = new Map(); // fixture_id -> { price, books_count, books_counts_raw, implied, ev }
  for (const rec of targets){
    ...
    // pick je već odlučen u rebuild-u
  ...
  for (const k of keysToPatch){
    ...
    for (const f of fields){
      ...
      for (const it of obj[f]){
        ...
        // ažuriraj samo till ovog "watcher" skupa
