...
export default async function handler(req, res) {
  try {
    ...
    const gen = await fetch(`${origin}/api/value-bets`, { headers: { "x-internal": "1" } });
    if (!gen.ok) {
    ...
    // 4) Post-steps: insights + jedan floats prolaz (best-effort)
    await fetch(`${origin}/api/insights-build`, { headers: { "x-internal": "1" } }).catch(()=>{});
    await fetch(`${origin}/api/locked-floats`, { headers: { "x-internal": "1" } }).catch(()=>{});
