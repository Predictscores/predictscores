...
// Zaključava predloge po slotu (vbl:<YMD>:<slot>) + upisuje dnevni seed (vb:day:<YMD>:<slot>)
...
// KV (apply-learning čita odavde)
const KV_URL   = process.env.KV_REST_API_URL;
const KV_TOKEN = process.env.KV_REST_API_TOKEN;

// Tier mape (Serbia: SuperLiga = T1; Prva Liga ide u T3)
const TIER1 = toSet([
  ...
  "Premier League","LaLiga","Serie A","Bundesliga","Ligue 1","Eredivisie","Primeira Liga","Pro League","Süper Lig","Super Lig","Premiership",
  "Austrian Bundesliga","Swiss Super League","SuperLiga","Serbian SuperLiga"
]);
const TIER2 = toSet([
...
function bestOdds(x){
  const b = Number(x?.market_odds) || Number(x?.odds?.best) || null;
...
// KV set za vb:day (TOP-LEVEL niz kroz JSON.stringify)
