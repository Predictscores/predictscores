...
export default async function handler(req, res) {
  const symbols = [ /* ovde ubaci top 50 simbola koje želiš pratiti */ 
  ...
  const results = await Promise.all(
    symbols.map(async symbol => {
      try {
        // 1) Minute bars (15m aggregat)
        let resMin = await fetch(
          `https://min-api.cryptocompare.com/data/v2/histominute?fsym=${symbol}&tsym=USD&limit=96&aggregate=15`
        );
        let jsonMin = await resMin.json();
        let bars = jsonMin.Data?.Data || [];
        if (bars.length < 10) {
          console.warn(`Few minute bars for ${symbol}, fallback to CG OHLC`);
        ...
        // Price info + basic signal logic (delta trend)
        ...
        return {
          ...
          change1h: ((latest.close - bars[bars.length-5].open)/bars[bars.length-5].open)*100,
          change24h: ((latest.close - bars[0].open)/bars[0].open)*100,
          entryPrice,
          sl,
          tp,
          expectedMove
