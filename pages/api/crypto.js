...
export default async function handler(req, res) {
  try {
    // 1) Fetch top 100 coins with price change %
    ...
    // 2) Process each coin with individual try/catch
    const items = await Promise.all(
      list.map(async c => {
        try {
          ...
          // 3) Fetch 240-minute history at 1m bars
          ...
          // 6) Calculate SMA crossovers per TF
          function crossover(dataPoints, shortLen, longLen) {
            if (dataPoints.length < longLen) return 0;
          ...
          // 7) Aggregate signals into a score
          ...
          // 8) Fetch hourly history for ATR
          let hourlyData = [];
          try {
            const hr = await fetch(
              `https://min-api.cryptocompare.com/data/v2/histohour?fsym=${symbol}&tsym=USD&limit=24`
            );
            const jh = await hr.json();
            hourlyData = jh.Data?.Data || [];
          } catch(err) {
            console.error('Error fetching hourly for', symbol, err);
          }

          // 9) Compute ATR on last 14 true ranges
          const trs = hourlyData.map((h,i,arr) => {
          ...
          const atr = sma(trs.slice(-14), 14)
                   ?? (hourlyData.length ? hourlyData[0].high - hourlyData[0].low : 0);

          // 10) Set SL/TP based on ATR
          let slPrice, tpPrice;
          if (signal === 'LONG') {
          ...
          console.error('Error processing', c.symbol, err);
          return null;
        }
      })
    );

    // 11) Filter out failed coins
    const results = items.filter(item => item !== null);

    // 12) Sort and slice
    results.sort((a, b) => b.confidence - a.confidence);
    const crypto  = results.slice(0, 10);
    const combined = crypto.slice(0, 3);

    // Response with caching
    res.setHeader('Cache-Control', 's-maxage=600, stale-while-revalidate');
    res.status(200).json({ combined, crypto });
  } catch(err) {
