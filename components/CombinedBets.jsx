...
const BAN_REGEX = /(U-?\d{1,2}\b|\bU\d{1,2}\b|Under\s?\d{1,2}|Reserves?|B\s*-?\s*Team|II\s*-?\s*Team|Youth|Women|Girls|Development|Academy)/i;
...
export default function CombinedBets({ initialFootball = [], initialCrypto = [] }) {
  ...
  // FOOTBALL — locked → fallback(24h) + BAN i short cache u fallbacku (120s)
  useEffect(() => {
    let stop = false;
    (async () => {
      // 1) locked
      ...
      // 2) fallback sa localStorage TTL 120s
      ...
      <div className="mt-6">
        ...
        {tab === "Football" && (
          <>
            ...
            {subTab === "History" && <HistoryPanel history={history} />}
          </>
        )}
        {tab === "Crypto" && (
          <div className="grid gap-3">
