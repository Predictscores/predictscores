...
function product(arr) {
  return arr.reduce((a, x) => (Number.isFinite(x) ? a * x : a), 1);
}
            ...
            {legs.map((l) => (
              <li key={l.id} className="flex items-start justify-between gap-3">
                <span className="min-w-0">
                  ...
                  <span className="text-slate-400">({l.sel || "—"})</span>
                </span>
...
/** Freeze tickets by (ymd,slot). Uses tickets from value-bets-locked. */
function useFrozenTickets() {
  ...
  async function load() {
    try {
      ...
      const t =
        j && typeof j === "object" && j.tickets && typeof j.tickets === "object"
          ? j.tickets
          : null;

      const normTickets = (raw) => ({
      ...
      if (t && (t.btts?.length || t.ou25?.length || t.htft?.length)) {
        const payload = { ...normTickets(t), meta: { ymd: j?.ymd || ymd, slot: j?.slot || slot } };
        try {
...
/* ===================== Football tab: 1X2 + Specials (tickets) ===================== */
function FootballBody({ list, tickets }) {
  const [tab, setTab] = useState("ko"); // ko | conf | hist

  // samo 1X2 za levi deo
  const oneX2All = useMemo(
    () => list.filter((x) => String(x.market).toUpperCase() === "1X2"),
    [list]
  );
  ...
  const confLeft = useMemo(
    () => [...oneX2All].sort((a, b) => b.conf - a.conf),
    [oneX2All]
  );

  const left = tab === "ko" ? koLeft : confLeft;

  // specials dolaze iz zamrznutih "tickets"
      ...
      {tab === "hist" ? (
        ...
        <div className="rounded-2xl bg-[#15182a] p-4">
          ...
          <div className="flex flex-col md:flex-row md:gap-4 gap-4">
            ...
            {/* Specials (45%) – tri tiketa */}
            <section className="md:basis-[45%] md:min-w-0">
              <div className="text-slate-200 font-semibold mb-2">
                Specials — BTTS / HT-FT / O/U 2.5
              </div>
    ...
    <div className="mt-4 space-y-4">
      ...
      {tab === "Combined" &&
        (fb.loading ? (
          ...
          <CombinedBody footballTop3={top3Football} cryptoTop3={[]} />
        ))}
      ...
      {tab === "Football" &&
        (fb.loading || tLoading ? (
          ...
          <div className="text-red-400 text-sm">Greška (tickets): {tErr}</div>
        ) : (
          <FootballBody list={fb.list} tickets={tickets} />
        ))}
