...
class ErrBoundary extends React.Component {
  ...
  static getDerivedStateFromError(e){ return { hasError:true, msg: String(e?.message||e) }; }
  componentDidCatch(e, info){ console.error("CombinedBets render error:", e, info); }
  render(){
    if (this.state.hasError){
      return (
        <div className="rounded-xl bg-[#1f2339] p-4 text-sm text-amber-300">
          Nešto je pošlo naopako u prikazu. Pokušaću sa bezbednim prikazom. ({this.state.msg})
        </div>
      );
    }
...
async function safeJson(url) {
  try {
    ...
    console.warn("safeJson fallback for", url, e);
    return {};
  }
}
function parseKickoff(it) {
  try{
    if (it?.kickoff_utc) {
      const d = new Date(it.kickoff_utc);
    ...
    const withTZ = /[Z+-]\d\d:?\d\d$/.test(s) ? s : (s + "Z");
...
function teamName(side) {
  ...
  if (typeof side === "object") return side.name || side.team || side.id || "—";
  return "—";
}
...
function normalizeBet(it) {
  try{
    ...
    let odds = null;
    if (typeof it?.odds === "object" && it?.odds) odds = Number(it.odds.price);
    else odds = Number(it?.market_odds ?? it?.odds);
    ...
    const id =
      it?.fixture_id ??
      it?.fixture?.id ??
      `${home}-${away}-${(date && !isNaN(date.getTime()) ? date.getTime() : Date.now())}`;

    return { id, league, date, home, away, market, sel, odds, conf, explain: it?.explain };
  }catch(e){
    console.warn("normalizeBet error:", e, it);
    return null;
  }
}
  ...
  return (
    <div className="p-4 rounded-xl bg-[#1f2339]">
      ...
      <div className="text-sm text-slate-200 mt-1">
        <span className="font-semibold">{bet?.market || "1X2"}</span>
        {bet?.market ? " → " : " "}
        {bet?.sel || "—"}
  ...
  async function load() {
    try {
      ...
      const arr =
        ...
        Array.isArray(j) ? j : [];
      const norm = arr.map(normalizeBet).filter(Boolean);
      setList(norm);
...
function FootballBody({ list }) {
  ...
  const oneX2All = useMemo(
    () => (Array.isArray(list) ? list : []).filter(x => String(x?.market).toUpperCase() === "1X2"),
    [list]
  );
  const specialsAll = useMemo(() => (Array.isArray(list) ? list : []).filter(x => {
  ...
  const confLeft = useMemo(() => [...oneX2All].sort((a, b) => (Number(b?.conf||0) - Number(a?.conf||0))), [oneX2All]);
  const confRight = useMemo(() => [...specialsAll].sort((a, b) => (Number(b?.conf||0) - Number(a?.conf||0))), [specialsAll]);
...
export default function CombinedBets() {
  ...
  const top3Football = useMemo(() => [...(fb.list||[])].sort((a, b) => (Number(b?.conf||0) - Number(a?.conf||0))).slice(0, 3), [fb.list]);
