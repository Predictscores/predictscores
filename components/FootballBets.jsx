...
export default function FootballBets({ date, limit, tall = false }) {
  ...
  const bucketForPct = (pct) =>
    pct >= 90 ? "TOP" : pct >= 75 ? "High" : pct >= 50 ? "Moderate" : "Low";
  ...
  const ensureConfidence = (b) => {
    ...
    return {
      ...b,
      confidence_pct: pct,
      confidence_bucket: b.confidence_bucket || (pct != null ? bucketForPct(pct) : null),
    };
  };
  ...
  const fetchBets = async () => {
    ...
    setError(null);
    try {
      const qs = new URLSearchParams({ date: date || "", min_edge: "0.05", min_odds: "1.3" });
  ...
  const list = useMemo(() => (limit ? bets.slice(0, Number(limit)) : bets), [bets, limit]);
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && (
        <div className="p-4 bg-white/5 rounded-lg border border-white/10">Loading predictions...</div>
      )}
      {error && (
          ...
          return (
            ...
            >
              ...
              <div className="mt-2 flex items-center gap-3 text-sm">
                ...
                <div className={`text-xs px-2 py-1 rounded ${badge}`}>{type}</div>
              </div>
              ...
              <div className="mt-2 text-xs text-white/70 flex flex-wrap gap-4">
                {model_prob != null && (
                  <div>
                    ...
                    <span className="text-white">{formatPercent(model_prob)}</span>
                  </div>
                )}
                {edge != null && (
                  <div>
              ...
              {/* spacer da bar zauzme visinu kad je tall */}
