...
// In-memory caches (cold start reset)
const fixturesCache = {};
...
const getTeamForm = async (teamId) => {
  ...
  if (!API_FOOTBALL_KEY) return { formValue: 0, recentMatches: [] };
...
/**
 * Returns top N match picks for given date (YYYY-MM-DD or undefined for today)
 */
const getTopMatches = async (dateOverride) => {
  const normalized = normalizeDateInput(dateOverride);
  const dateStr = normalized || getTodayDateStr();
