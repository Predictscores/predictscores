...
import { upstashFallbackIncr } from "./upstash-fallback";
...
export async function reserveCoinGeckoQuota(now = Date.now()) {
  ...
  if (!url || !token) {
    const minuteCount = upstashFallbackIncr(minuteKey, { expireSeconds: 120, now });
    const dayCount = upstashFallbackIncr(dayKey, { expireSeconds: 172800, now });
    const snapshot = {
